...
class FrontendController extends Controller
{
    ...
    public function postcomment(Request $request,$id){
        if (Auth::check()) {
            ...
            alert()->warning('Bạn cần đăng nhập để có thể bình luận');
            return back();
        }
    ...
    public function poststar(Request $request,$id){
        ...
        // if (Auth::check()) {
            ...
            if (rating::where('name_id',Auth::user()->name_id)->exists() && rating::where('prod_id',$id)->exists()) {
                ...
                alert()->success('Cảm ơn', 'Bạn đã đánh giá sản phẩm của chúng tôi');
                // echo 'thanhcong'; 
                // return back();
            }
            else {
    ...
    public function postpassword(Request $request ,$id){
        $rules = [
        'password' => 'min:8|confirmed|max:15',
        'old_password'=> 'min:8|max:15'
        ];
        $messages = [
